...
setTimeout(function() {
  try {
    ...
    title.style.textShadow =
      "0 0 2px #393a33, 0 0 35px #ffffff44, 0 0 8px #f39f0575, 0 0 2px #f39f0575";
...
function applyStyles() {
  // Anchors
  queryShadow(document, el => el.tagName === "A").forEach(
    el => (el.style.color = primaryColor)
  );

  // Label classes eg. https://www.home-assistant.io/lovelace/entities/#label
  queryShadow(document, el => el.className === "label").forEach(
    el => (el.style.color = primaryColor)
  );

  // Navigation Menu Icon
  queryShadow(document, el => el.className === "menu").forEach(el => {
...
function detectHacs() {
  ...
  if (rootEl && rootEl.shadowRoot) {
    const homeAssistantMainEl = rootEl.shadowRoot.querySelector(
      "home-assistant-main"
    );

    if (homeAssistantMainEl && homeAssistantMainEl.shadowRoot) {
      const appDrawerLayoutEl = homeAssistantMainEl.shadowRoot.querySelector(
        "app-drawer-layout"
      );

      if (appDrawerLayoutEl) {
        const haPanelIframeEl = appDrawerLayoutEl.querySelector(
          "ha-panel-iframe"
        );
...
function applyHacsStyles(hacsIframe) {
  ...
  function waitForIframeAndApply(hacsIframe) {
    ...
    var iframeDoc =
      hacsIframe.contentDocument || hacsIframe.contentWindow.document;
